...
def dealer_actions():
    ...
    while dealer_total < dealer_hits_soft:
        ...
        print(
            f"Dealer hits.  Dealer cards: {dealer_cards}, current total: {dealer_total}"
        )
        if dealer_total > 21:
        ...
        print(
            f"Dealer stands.  Dealer cards: {dealer_cards}, current total: {dealer_total}"
        )
        determine_winner()
...
if answer == 'y':
    ...
    shuffle_point = float(
        input(
            "At what point would you like the shoe to be shuffled? (e.g. 75% of cards in the shoe have been dealt, insert as '75') "
        )) / 100
    while game_running:
        your_cards = []
        dealer_cards = []
        ...
        dealer_cards.append(
            deal_one_card())  #First card dealt in hand to dealer
        your_cards.append(deal_one_card())  #Second card dealt in hand to you
        dealer_cards.append(
            deal_one_card())  #Second card dealt in hand to dealer
        your_total = sum(your_cards)
        dealer_total = sum(dealer_cards)
        ...
        if dealer_total == 21:
            print(
                f"Dealer has blackjack. {dealer_cards}. Dealer wins. Game over."
            )
        elif your_total == 21:
            ...
            if hit_or_stand == 's':
                dealer_actions()
        cards_dealt = len(your_cards) + len(dealer_cards)
        cumulative_cards_dealt += cards_dealt
        percent_of_shoe_used = cumulative_cards_dealt / cards_in_shoe
